...
CCSE.launch = function(){
	...
	Update history
	=======================================================================================*/
	{	CCSE.updateLog = '<div class="subsection"><div class="listing">Cookie Clicker Script Extender 是一个模组框架，为的是让编写模组更简便。</div>' +
		'<div class="listing">作者与维护者均为 Klattmose (<a href="https://github.com/klattmose" target="_blank">GitHub</a>, <a href="https://www.reddit.com/user/klattmose/" target="_blank">reddit</a>)</div>' +
		'<div class="listing">更多信息请见<a href="https://klattmose.github.io/CookieClicker/CCSE-POCs/" target="_blank">这里</a>。</div>' +
		'<div class="listing">问题反馈请去<a href="https://github.com/klattmose/klattmose.github.io/issues" target="_blank">这里</a>。</div></div>' +
	...
	CCSE.InjectCodeIntoFunction = function(functionName, alterFunctionCode, code, preEvalScript, hasPrototype){
		...
		if (originalFunction === null) {
			console.warn(`CCSE: ${ functionName } 未找到。请不要注入 ${ code }`);
			return;
		} else if (typeof originalFunction !== "function") {
			console.warn(`CCSE: ${ functionName } 不是函数。请不要注入 ${ code }`);
			return;
		}
	...
	CCSE.InitNote = function(){
		...
		Game.Notify('CCSE 正在初始化', CCSE.GetProgressHTML(0), [0, 0, CCSE.iconURL], 6, 1);
	...
	CCSE.GetMenuString = function(){
		var str = '<div class="listing">' +
			CCSE.MenuHelper.ActionButton("CCSE.ExportSave();", '导出自定义存档') +
			CCSE.MenuHelper.ActionButton("CCSE.ImportSave();", '载入自定义存档') +
			'<label>模组添加的备份数据，可以由 CCSE 管理</label></div>';

		str += '<div class="listing">' + CCSE.MenuHelper.CheckBox(CCSE.config, 'showVersionNo', 'showVersionNoButton', '显示版本号', '不显示版本号', 'CCSE.togglePref') + '<label>在屏幕左下角显示 CCSE 版本号。</label></div>';
...
`, preEvalScript);
	...
	CCSE.ExportSave = function(){
		Game.Prompt(
			'<h3>导出配置</h3>' +
			'<div class="block">这是你的 CCSE 存档。<br>其中也包括了其他作者开放让 CCSE 管理的模组数据，以及通过 CCSE 可以自定义的内容（比如成就、升级等等）</div>' +
	...
	CCSE.ImportSave = function(){
		...
		Game.Prompt(
			'<h3>载入配置</h3>'+
			'<div class="block">在这里粘贴你的 CCSE 存档。</div>'+
	...
	CCSE.ConfirmLoad = function(modName, modVersion, versionText){
		return confirm(
			`${ modName } 版本 ${ modVersion } 是为 ${ versionText }设计的。`+
			"用其他版本加载可能会出错。" +
			`你确定仍然要加载 ${ modName } 吗？`);
	}
	CCSE.ConfirmGameVersion = function(modName, modVersion, version){
		...
		if(Game.version != version){
			proceed = CCSE.ConfirmLoad(modName, modVersion, `游戏版本 ${ version }`);
		}
		return proceed;
	}
	...
	CCSE.ConfirmCCSEVersion = function(modName, modVersion, version){
		...
		if(CCSE.version != version){
			proceed = CCSE.ConfirmLoad(modName, modVersion, `CCSE 版本 ${ version }`);
		}
		return proceed;
	}
	...
	CCSE.ConfirmGameCCSEVersion = function(modName, modVersion, gameVersion, ccseVersion){
		...
		if(Game.version != gameVersion && CCSE.version != ccseVersion){
			proceed = CCSE.ConfirmLoad(modName, modVersion, `游戏版本 ${ gameVersion } 和 CCSE 版本 ${ ccseVersion }`);
		}
		else if(Game.version != gameVersion){
			proceed = CCSE.ConfirmLoad(modName, modVersion, `游戏版本 ${ gameVersion }`);
		}
		else if(CCSE.version != ccseVersion){
			proceed = CCSE.ConfirmLoad(modName, modVersion, `CCSE 版本 ${ ccseVersion }`);
		}
		return proceed;
	}
